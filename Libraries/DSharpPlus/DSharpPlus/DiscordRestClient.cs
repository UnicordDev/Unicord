...
namespace DSharpPlus
{
    public class DiscordRestClient : BaseDiscordClient
    {
        ...
        public Task ModifyChannelAsync(ulong id, string name, int? position, string topic, bool? nsfw, Optional<ulong?> parent, int? bitrate, int? user_limit, Optional<int?> perUserRateLimit, string reason)
            => ApiClient.ModifyChannelAsync(id, name, position, topic, nsfw, parent, bitrate, user_limit, perUserRateLimit, reason);
        ...
        public async Task ModifyAsync(ulong member_id, ulong guild_id, Action<MemberEditModel> action)
        {
            ...
            {
                await ApiClient.ModifyGuildMemberAsync(guild_id, member_id, mdl.Nickname,
                    mdl.Roles.IfPresent(e => e.Select(xr => xr.Id)), mdl.Muted, mdl.Deafened,
                    mdl.VoiceChannel.IfPresent(e => e?.Id), mdl.AuditLogReason).ConfigureAwait(false);
            }
        }
